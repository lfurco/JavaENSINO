...
export default class DoublyLinkedList{
    ...
    insert(pos, val){
        ...
        // 2º CASO: inserção no INÍCIO (head)
            ...
            this.#head.prev = inserted          // antigo head aponta para o novo
        ...
        // 3º CASO: inserção no FINAL
            ...
            this.#tail.next = inserted          // antigo tail aponta para o novo
    ...
    peekTail(pos){
        ...
        indexOf(val){ // Pra saber a posicao do indice que estou buscando

            // Calcula o meio da lista (otimização: busca dos dois lados)
            const middle = Math.ceil(this.#count / 2)
            let node1 = this.#head  // Ponteiro começando do início (head)
            let node2 = this.#tail  // Ponteiro começando do final (tail)


            // Percorre até a metade da lista
            for(let pos = 0; pos < middle; pos++){
                // verifica se o valor está no node1    (head)
                if(val === node1.data) return pos
                // Verifica se o valor está no node2    (tail)
                if(val === node2.data) return this.#count - 1 - pos

                // Avança o ponteiro da frente para o próximo nó
                node1 = node1.next

                // Retrocede o ponteiro de trás para o nó anterior
                node2 = node2.prev
            }
        ...
        print(){
            ...
            for(let i = 0; i < this.#count; i++){
                if(output !== '( ') output += ', '  // Se não for vazio é pq tem alguem, então: ','
                output += `[${i}]: ${node.data}`    // Adiciona o índice e o valor do nó atual
